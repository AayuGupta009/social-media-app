...
import ModalVol from "./ModalVol";
import { useState } from "react";
import { useDispatch, useSelector } from "react-redux";
import { userVolDataViewAction } from "../../../../../redux/action/user-management/user-details/userDetailsAction";
import { formatDate } from "../../../../../utils/commonFunctions";

export default function UserVolAppnTable({
  rows,
  page,
  rowsPerPage,
  ...
  const handleOpen = (appnId) => {
    setOpen(true);
    dispatch(userVolDataViewAction(appnId, params?.id));
  };

  const applicationData = useSelector(
    (store) => store.userVolReqViewReducer?.userVolData
  );
  return (
    <>
      <TableContainer>
      ...
      />
      <CustomDialog
        header={t("modal_title.volunteer")}
        header_style="custom_header"
        bodyChild={<ModalVol datas={applicationData?.result} />}
        isOpen={open}
        isButton={true}
