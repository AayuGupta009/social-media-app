...
import VolModalContent from "./VolModalContent";
...
import { viewVolApplicantsDataAction } from "../../../redux/action/volunteer-management/volunteerAction";
import { useParams } from "react-router-dom";
import { formatDate } from "../../../utils/commonFunctions";

export default function ViewVolApplicantsListTable({
  rows,
  page,
  rowsPerPage,
  ...
  const columns = [
    ...
    { id: "dateOfBirth", label: `${t("table.dob")}`, minWidth: 100 },
    {
      id: "gender",
      label: `${t("modal_content.gender")}`,
      minWidth: 170,
      align: "left",
    },
    {
  ...
  const handleOpen = (appnId) => {
    setOpens(true);
    dispatch(viewVolApplicantsDataAction(params?.id, appnId));
  };

  const applicationData = useSelector(
    (store) => store.viewVolApplicantsDataReducer?.viewData
  );
  return (
    <Paper sx={{ width: "100%", overflow: "hidden" }}>
      ...
      <CustomDialog
        header={t("modal_content.vol_applicants")}
        header_style="custom_header"
        bodyChild={<VolModalContent datas={applicationData?.result} />}
        isOpen={opens}
        isButton={true}
